...
impl TaskManager {
    ...
    fn add_current_syscall_num(&self, syscall_id: usize) {
        let mut inner = self.inner.exclusive_access();
        let current = inner.current_task;
    ...
    pub fn get_current_task_syscall_times(&self) -> Option<[u32; MAX_SYSCALL_NUM]> {
        let inner = self.inner.exclusive_access();
        Some(inner.tasks[inner.current_task].task_syscall_times.clone())
    }

    pub fn get_current_running_time(&self) -> usize {
        let inner = self.inner.exclusive_access();
        inner.tasks[inner.current_task].task_running_time - inner.tasks[inner.current_task].task_time_first_run
    }
}
...
pub fn record_time(ms: usize) {
    TASK_MANAGER.add_current_running_time(ms);
}

pub fn get_current_task_info() -> (TaskStatus, [u32; MAX_SYSCALL_NUM], usize){
    (
        match TASK_MANAGER.get_current_task_status() {
            Some(status) => status,
            _ => TaskStatus::UnInit
        },
        match TASK_MANAGER.get_current_task_syscall_times() {
